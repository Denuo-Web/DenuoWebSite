...
Current public positioning: Denuo Web is Jaron Rosenau's independent software-delivery and portfolio umbrella for practical API-driven web apps, integrations, civic/public-systems tooling, marketplace workflows, internal tools, operational tooling, and supportable launches. It should be presented as an umbrella for project evidence and delivery work, not as conventional employment history. Public case studies are organized around three offers: Discovery / Implementation Plan, Web App / Integration Build, and Stabilize / Support Handoff.
...
## Content model
- Live content stored at Firestore `siteContent/public`; case studies are stored as documents at `siteContent/public/work/{slug}`. Fallback content in `web/src/content/fallback.ts` is built from Jaron Rosenau's current positioning and recent work (QuestByCycle, CrowdPM Platform, Moonshine Art, APK Workbench, DripCopy).
